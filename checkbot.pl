...
#
# $Id: checkbot.pl,v 1.55 1999/05/09 15:30:34 graaff Exp $
...
sub handle_url {
  ...
  if (defined($url->scheme) 
	...
	my $redir_url = new URI::URL $response->header('Location'), $url;
...
sub handle_doc {
  ...
  foreach (@links) {
    ...
    foreach (keys %l) {
      ...
      if ($url->scheme =~ /^(http|ftp)/ and $url->host !~ /\./) {
        print STDERR "--> Unqualified host name: $url\n";
        $warning{'Unqualified host name in URL'}{$url} .= $url->base . "\n";
      }
